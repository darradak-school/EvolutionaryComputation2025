...
# pulls both MultiEA and SingleEA implementations
...
def run_experiments_with_tracking(problem_id, pop_size, num_runs=30):
    ...
    for run in range(num_runs):
        ...
        while evals < max_evals:
            # ... (evolution code here - simplified for brevity)
...
def create_fixed_budget_plot(problem_id, save_path='plots/'):
    ...
    for pop_size in pop_sizes:
        ...
        histories, _ = run_experiments_with_tracking(problem_id, pop_size, num_runs=5)
...
def generate_all_plots():
    ...
    print("-"*40)
    for problem_id in max_coverage[:2]:  # Just first 2 for demo
        create_fixed_budget_plot(problem_id)
    
    for problem_id in max_influence[:2]:  # Just first 2 for demo
        create_fixed_budget_plot(problem_id)
    
    # 2. Generate Pareto front plots
    print("\n2. Creating Pareto Front Plots...")
    print("-"*40)
    for problem_id in max_coverage[:2]:
        create_pareto_front_plot(problem_id)
    
    for problem_id in max_influence[:2]:
        create_pareto_front_plot(problem_id)
...
if __name__ == "__main__":
    ...
    # Example: Export for IOHanalyzer
    # export_for_iohanalyzer(2100, 20, "ioh_data_2100.csv")
    
    print("\nFor IOHanalyzer:")
    print("1. Go to https://iohanalyzer.liacs.nl/")
    print("2. Upload the CSV files generated by export_for_iohanalyzer()")
    print("3. Use the web interface to create interactive plots")